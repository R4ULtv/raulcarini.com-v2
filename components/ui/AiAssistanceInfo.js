...
export default function AiAssistanceInfo() {
  return (
    <Popover as={Fragment}>
      <PopoverButton className="outline-none text-zinc-800 dark:text-zinc-200 p-1 group">
        <SparklesIcon className="size-4 group-hover:scale-110 transition duration-75 ease-out" />
      </PopoverButton>
      <PopoverPanel
        transition
        anchor="bottom"
        className="p-3 my-1 w-52 space-y-1.5 text-zinc-700 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl transition ease-out data-[closed]:-translate-y-8 data-[closed]:opacity-0 data-[closed]:scale-50"
      >
