...
export type PostSummary = {
  slug: string; // relative path inside content/ without .mdx, using forward slashes
...
export async function getAllPosts(): Promise<PostSummary[]> {
  const entries = fs.readdirSync(CONTENT_DIR, { recursive: true, withFileTypes: true });
  ...
  const posts = await Promise.all(
    mdxFiles.map(async (entry) => {
      ...
      const mod = (await import(`@/content/${slug}.mdx`)) as { metadata: PostMetadata };
